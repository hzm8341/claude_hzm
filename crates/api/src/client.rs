...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        // Check MINIMAX_API_KEY first, then fall back to ANTHROPIC_API_KEY
        let api_key = read_env_non_empty("MINIMAX_API_KEY")
            .or_else(|_| read_env_non_empty("ANTHROPIC_API_KEY"))?;
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        // Check MINIMAX_API_KEY first, then fall back to ANTHROPIC_API_KEY
        if let Some(api_key) = read_env_non_empty("MINIMAX_API_KEY")
            .or_else(|_| read_env_non_empty("ANTHROPIC_API_KEY"))?
        {
